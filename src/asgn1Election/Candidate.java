...
public class Candidate {
	...
	public Candidate(String candName, String candParty, String candAbbrev, int voteCount) throws ElectionException {
		// Candidate Name Exception
		if (candName != null && candName != "") {
			this.name = candName;
		} else {
			throw new ElectionException("Candidate Name is null or empty");
		}
		// Candidate Party Exception
		if (candParty != null && candParty != "") {
			this.party = candParty;
		} else {
			throw new ElectionException("Candidate Party is null or empty");
		}
		// Candidate Abbreviation Exception
		if (candAbbrev != null && candAbbrev != "") {
			this.abbrev = candAbbrev;
		} else {
			throw new ElectionException("Candidate Abbreviation is null or empty");
		}
		if (voteCount >= 0) {
			this.voteCount = voteCount;
		} else {
			throw new ElectionException("Vote Count below 0");
		}
	}
